...
interface DemoSession {
  ...
  capture_mode: 'polling' | 'motion' | 'manual' | 'yoloworld';
...
const DemoStreamPage: React.FC = () => {
  ...
  const [captureMode, setCaptureMode] = useState<'polling' | 'motion' | 'manual' | 'webcam' | 'yoloworld'>('polling');
  ...
  const startSession = async () => {
    // Determine if IntelliSearch will use webcam or stream URL
    const yoloworldUseWebcam = captureMode === 'yoloworld' && !youtubeUrl.trim();
    const yoloworldUseStream = captureMode === 'yoloworld' && youtubeUrl.trim();

    // Webcam mode doesn't need a URL, but other modes do (except yoloworld with webcam)
    if (captureMode !== 'webcam' && !yoloworldUseWebcam && !youtubeUrl) {
      toast.error('Please enter a stream URL');
      return;
    }
    // YOLOWorld mode needs prompts instead of detectors
    if (captureMode === 'yoloworld') {
    ...
    // For webcam mode or yoloworld without URL, start the camera first
    if (captureMode === 'webcam' || yoloworldUseWebcam) {
    ...
    try {
      ...
      if (captureMode === 'webcam' || yoloworldUseWebcam) {
        // Webcam or IntelliSearch with webcam
        sessionUrl = 'webcam://local';
        sessionCaptureMode = 'manual';
      } else if (yoloworldUseStream) {
        // IntelliSearch with stream URL - use polling for server-side capture
      ...
      const sessionData: any = {
        ...
        detector_ids: captureMode === 'yoloworld' ? [] : selectedDetectors,
      };

      // Add YOLOWorld prompts if in that mode
      if (captureMode === 'yoloworld') {
      ...
      if (yoloworldUseStream) {
        toast.success('IntelliSearch stream demo started!');
      } else if (captureMode === 'yoloworld') {
        toast.success('IntelliSearch webcam demo started!');
      } else {
      ...
      // For webcam/yoloworld-webcam mode, start client-side capture; otherwise poll for server frames
      if (captureMode === 'webcam' || yoloworldUseWebcam) {
  ...
  const captureWebcamFrame = async () => {
    ...
    // Check if this is a YOLOWorld session (no detectors, has prompts)
    const isYoloWorld = session.yoloworld_prompts || (detectorIds.length === 0 && prompts);

    if (isYoloWorld) {
      // YOLOWorld mode - send to special endpoint with prompts
      console.log(`Capturing webcam frame for YOLOWorld with prompts: ${prompts}`);
      try {
        await axios.post(`/demo-streams/sessions/${session.id}/submit-yoloworld-frame`, {
          image_data: base64Data,
          prompts: prompts,
          capture_method: 'yoloworld',
        });
      } catch (err) {
        console.error('YOLOWorld frame submission error:', err);
      }
    } else {
      ...
      {!activeSession && (
        <div className="bg-gray-800 rounded-lg p-6 mb-6">
          ...
          <div className="space-y-4">
            ...
            {(captureMode === 'polling' || captureMode === 'webcam' || captureMode === 'yoloworld') && (
              <div>
                <label className="block text-sm font-medium text-gray-400 mb-2">
          ...
          <div className="bg-gray-800 rounded-lg p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-white">
                {activeSession.yoloworld_prompts ? '🔍 IntelliSearch Stream' : 'Stream'}
              </h2>
              <button
                onClick={stopSession}
            ...
            {/* IntelliSearch live prompt input - only show when yoloworld_prompts has a value */}
            {activeSession.yoloworld_prompts && (
              <div className="mb-4 p-3 bg-purple-900/30 border border-purple-700 rounded">
                ...
                <input
                  ...
                  className="w-full rounded-md bg-gray-700 border-purple-600 text-white p-2"
                />
              </div>
            )}
