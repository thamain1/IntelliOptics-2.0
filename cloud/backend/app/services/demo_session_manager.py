...
class DemoSessionManager:
            ...
            try:
                ...
                try:
                    ...
                    blob_path = upload_blob(
                        settings.blob.container_name,
                        blob_name,
                        image_bytes,
                        "image/jpeg"
                    )

                    # Check if this is a YOLOWorld session
                    if yoloworld_prompts:
                        ...
                        # Trigger YOLOWorld inference
                        thread = Thread(
                            target=process_yoloworld_inference,
                            args=(query.id, result.id, image_bytes, yoloworld_prompts),
                            daemon=True
                        )
                        thread.start()
                        ...
                        logger.info(
                            ...
                            f"(prompts: {yoloworld_prompts[:50]}...)"
                        )
    ...
    def stop_session(self, session_id: str) -> None:
        ...
        self._latest_frames.pop(session_id, None)  # Clean up stored frame
        if capture:
